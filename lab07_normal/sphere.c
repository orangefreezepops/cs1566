 ...
 * kdf11@pitt.edu 
 * 
 * Project 7: visualizing normals of 3D object
 *
 * compile: > make
 * run: ./sphere
 *
...
vec4 vertices[98340];
vec4 normals[98340];
int num_vertices = 98340; //(8 sides of octohedron) * (4096 triangles per side) * (3 vertices) = 98304 + 36 for cube
...
vec4 normal(vec4 t, vec4 br, vec4 bl){
	//generates normalized RGB values for coloring 
	vec4 v1, v2, normal;
	v1 = sub_vec(br, bl);
	v2 = sub_vec(t, br);
...
//vertex and normal arrays' placeholder
int v_index = 0;
...
void triangle(vec4 top, vec4 bottom_right, vec4 bottom_left){
	...
	vec4 curr = normal(top, bottom_right, bottom_left);
	normals[v_index] = curr;
